...
def format_offset(offset):
    ...
    # 根据偏移量大小使用不同颜色
    if offset > 0.3:
        return f"[bold red]{offset:.3f}m[/bold red]"
    elif offset > 0.15:
        return f"[yellow]{offset:.3f}m[/yellow]"
    else:
        return f"[green]{offset:.3f}m[/green]"
...
def create_display_table(data, packet_count, fps, tracking_enabled=False, state_just_changed=False):
    ...
    table.add_column("总偏移", style="magenta", width=12)
    
    # 获取总偏移值
    total_offset = data.get("totalOffset", 0.0)
        ...
        with Live(console=console, refresh_per_second=30) as live:
            while True:
                try:
                    ...
                    # 构建状态行
                    total_offset = data.get('totalOffset', 0.0)
                    offset_display = format_offset(total_offset)
                    status_text = f"Timestamp: {data.get('timestamp', 'N/A'):.3f}s | 总偏移: {offset_display} | From: {addr[0]}:{addr[1]}"
